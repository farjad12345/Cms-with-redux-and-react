...
import  { removeIteam, addUser } from "../../Redux/store/users";
...
export default function UserItem({ firstname, lastname,username,city, age, email,_id  }) {
  ...
  const removeHandler = () => {
    swal({
      title: "ایا از حذف مظمين هستید",
      icon: "warning",
      buttons: ['خیر', 'بله'],
    })
      .then((result) => {
        if (result) {
          dispatch(removeIteam(_id))
          swal({
  ...
  return (
    ...
    <div className="uesrs__item">
      ...
      <div className="users__btns">
        <button className="btn-custome btn-custome--gray" onClick={addUserHandler} >Add User</button>
        <button className="btn-custome btn-custome__blue" onClick={()=>setIsShowModal(prevstate => !prevstate)}>جزیيات</button>
